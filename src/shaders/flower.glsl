...
void main() {
  ...
  float j=.0;
  for(int i=0;i<12;i++){
    dir=vec2(sin(tau*(frame+j)/speed),cos(tau*(frame+j)/speed));
    col=line(col,vec2(.0),dir*.7,q1,vec2(.015,1.),vec4(173./255.,127./255.,1.,1.));
    j+=5.*space;
  }
  j=.0;
  for(int i=0;i<12;i++){
    dir=vec2(cos(tau*(frame+j)/speed),sin(tau*(frame+j)/speed));
    col=line(col,vec2(.0),dir*.4,q2,vec2(.015,1.),vec4(1., 127./255.,1.,1.));
    j+=5.*space;
  }
  j=5.*space/2.;
  for(int i=0;i<12;i++){
